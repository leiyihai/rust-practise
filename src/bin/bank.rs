...
impl Bank {
    ...
    fn create_acc(&mut self, acc_id: u32, name: &str) {
        match self.accounts.iter_mut().find(|a| a.acc_id == acc_id ) {
            ...
            None => {
                self.accounts.push(Account { acc_id, owner: name.to_string(), balance: 0.0 });
...
fn main() {
    ...
    match bank.query_acc(1) {
        Some(acc) => { println!("查询到账户：{}", acc.acc_id) },
        None => println!("此账户不存在"),
    }
    println!("----查询不存在账户----");
    let search_id: u32 = 999;
    match bank.query_acc(search_id) {
        Some(acc) => { println!("查询到账户：{}{}",acc.owner, acc.acc_id) },
    ...
    match bank.deposit(1, 1_0000_0000.0) {
        Ok(data) => { println!("已存入：￥{}，当前余额：￥{}", data.0, data.1) },
        Err(e) => println!("{}", e),
    }
    println!();
